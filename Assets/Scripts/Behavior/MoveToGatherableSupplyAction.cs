...
[NodeDescription(name: "Move to GatherableSupply", story: "[Agent] moves to [Supply] or nearby not busy supply.", category: "Action/Navigation", id: "5aede66bc1a4a940bfcc1a19c76bdd8a")]
public partial class MoveToGatherableSupplyAction : Action
{
    ...
    protected override Status OnUpdate()
    {
        ...
        if (agent.remainingDistance >= agent.stoppingDistance) return Status.Running;

        if (Supply.Value != null && !Supply.Value.IsBusy && Supply.Value.Amount > 0) return Status.Success;
    ...
    {
        return Physics.OverlapSphere(agent.transform.position,
            SearchRadius,
            suppliesMask).Where(collider => collider.TryGetComponent(out GatherableSupply supply)
            && !supply.IsBusy
            && supply.Supply.Equals(supplySO)).ToArray();
    }

    private bool HasValidInputs()
    {
        if (!Agent.Value.TryGetComponent(out agent) || Supply.Value == null && supplySO == null) return false;
